  ...
  return (
    <AppShell>
      <PageHeader
        ...
        subtitle={
          lang === "ar"
            ? "إدارة المستخدمين والطلبات والتقارير"
            : "Manage users, requests, and reports"
        }
      >
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        ...
        />
        <StatCard
          label={lang === "ar" ? "إيداعات معلّقة" : "Pending deposits"}
          value={pendingD}
          icon={<Activity className="h-5 w-5" />}
          accent="emerald"
        />
        <StatCard
          label={lang === "ar" ? "سحوبات معلّقة" : "Pending withdrawals"}
          value={pendingW}
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-[1fr_auto]">
        <div className="flex items-center gap-3 rounded-full border border-border/70 bg-background/40 px-4 py-2.5">
          ...
          <input
            ...
            placeholder={
              lang === "ar"
                ? "ابحث بالاسم، البريد، الهاتف أو الوسيلة…"
                : "Search by name, email, phone or method…"
            }
        ...
        <Tabs<TabKey>
          ...
          tabs={[
            {
              key: "overview",
              label: lang === "ar" ? "النظرة العامة" : "Overview",
            },
            { key: "users", label: lang === "ar" ? "المستخدمون" : "Users" },
            {
              key: "deposits",
              label: lang === "ar" ? "الإيداعات" : "Deposits",
            },
            {
              key: "withdrawals",
              label: lang === "ar" ? "السحوبات" : "Withdrawals",
            },
      ...
      {tab === "overview" && (
        <div className="mt-6 grid gap-4 lg:grid-cols-3">
          ...
          <div className="rounded-3xl glass p-5 sm:p-6">
            <h3 className="font-display text-base text-ivory sm:text-lg">
              {lang === "ar" ? "الباقات المفعّلة" : "Packages"}
            </h3>
            <p className="text-xs text-muted-foreground">
              {lang === "ar"
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 text-xs text-muted-foreground">
                        {pkg ? (lang === "ar" ? pkg.nameAr : pkg.nameEn) : "—"}
                      </td>
                      <td className="py-3 text-xs font-semibold text-gold">
                        {fmtMoney(u.balance, lang)}
                      </td>
                      <td className="py-3">
                        <Badge
        ...
        <div className="mt-6 rounded-3xl glass p-5 sm:p-6">
          ...
          <div className="mt-4 space-y-3">
            {logs.map((l) => (
              <div
                key={l.id}
                className="flex items-start gap-3 rounded-2xl border border-border/40 p-3"
              >
                <span className="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-gold" />
                <div className="min-w-0">
                  <p className="text-sm text-ivory">{l.action}</p>
                  <p className="text-[11px] text-muted-foreground">
                    {l.actor} — {fmtDateTime(l.at)}
                  </p>
                </div>
              </div>
            ))}
...
function AdjustBalanceBtn({
  ...
  const apply = (sign: 1 | -1) => {
    ...
    adjustBalance(
      userId,
      sign * n,
      reason.trim() || (lang === "ar" ? "تعديل يدوي" : "Manual adjustment"),
    );
    setOpen(false);
    setDelta("");
    setReason("");
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="grid h-8 w-8 place-items-center rounded-full border border-gold/40 text-gold transition-colors hover:bg-gold/10"
      ...
      {open && (
        <div className="absolute end-0 top-10 z-30 w-64 rounded-2xl glass p-4 shadow-2xl">
          ...
          <input
            ...
            inputMode="numeric"
            dir="ltr"
            className="mt-3 w-full rounded-xl border border-border/60 bg-background/40 px-3 py-2 text-sm text-ivory outline-none focus:border-gold/60"
          />
          <input
            value={reason}
            ...
            className="mt-2 w-full rounded-xl border border-border/60 bg-background/40 px-3 py-2 text-sm text-ivory outline-none focus:border-gold/60"
          />
          <div className="mt-3 grid grid-cols-2 gap-2">
            <button
              onClick={() => apply(1)}
              className="flex items-center justify-center gap-1 rounded-xl border border-success/40 py-2 text-xs font-semibold text-success hover:bg-success/10"
            >
              <Plus className="h-3.5 w-3.5" /> {lang === "ar" ? "إضافة" : "Add"}
            </button>
            <button
              onClick={() => apply(-1)}
              className="flex items-center justify-center gap-1 rounded-xl border border-destructive/40 py-2 text-xs font-semibold text-destructive hover:bg-destructive/10"
            >
          ...
          >
            <div className="min-w-0">
              <p className="truncate text-sm font-medium text-ivory">
                {r.userName} — {fmtMoney(r.amount, lang)} ج.م
              </p>
              <p className="truncate text-[11px] text-muted-foreground">
                {r.method} — {fmtDateTime(r.createdAt)}
              </p>
              {r.notes && (
