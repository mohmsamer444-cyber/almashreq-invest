  ...
  ({ className, variant = "gold", size = "md", fullWidth, ...props }, ref) => {
    return (
      <button
        ref={ref}
        className={cn(
          "shine inline-flex items-center justify-center gap-2 rounded-full font-semibold transition-all duration-300 active:scale-[0.97] disabled:opacity-60 disabled:cursor-not-allowed",
          "hover:-translate-y-0.5 hover:shadow-lg hover:brightness-[1.04]",
          ...
          variant === "outline" &&
            "border border-border/70 text-foreground hover:border-gold/50 hover:text-gold hover:bg-gold/5",
          variant === "ghost" &&
          ...
          variant === "success" &&
            "border border-success/40 text-success hover:bg-success/10 hover:border-success/60",
          className,
        )}
        style={
          variant === "gold"
            ...
            : variant === "emerald"
              ? {
                  backgroundImage: "var(--gradient-emerald)",
...
export function CardTitle({
  ...
  return (
    <h3 className={cn("font-display text-lg text-ivory", className)}>
      {children}
    </h3>
  );
}
...
export function CardContent({
  ...
  return (
    <div className={cn("px-6 pb-6 pt-4 sm:px-7 sm:pb-7", className)}>
      {children}
    </div>
  );
}
...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ className, label, error, icon, trailing, id, ...props }, ref) => {
    const inputId =
      id ?? (label ? `field_${label.replace(/\s+/g, "_")}` : undefined);
    return (
...
export function Tabs<T extends string>({
  ...
  return (
    <div
      className={cn(
        "flex gap-1 rounded-full border border-border/60 p-1",
        className,
      )}
    >
      {tabs.map((t) => (
...
export function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-14 text-center">
      <div className="grid h-16 w-16 place-items-center rounded-2xl glass text-muted-foreground">
        {icon}
      </div>
